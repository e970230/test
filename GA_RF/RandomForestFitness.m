...
tr_trainData=trainData(:,nov);   %訓練集 (使用GA選擇的特徵); 維度=sample數*所選特徵數(numFeats)
tr_vaildData=validData(:,nov);   %驗證集 (使用GA選擇的特徵); 維度=sample數*所選特徵數(numFeats)
...
predictions = predict(treeBaggerModel, tr_vaildData);  %以驗證集進行預測 
