function [fitness] = RandomForestFitness(params,data,labels,Split_quantity,indices, nov,RF_mode,k_fold_switch)
...
if k_fold_switch==1  %啟動k-fold功能
    ...
    if out_regression==1    %字串相符時確認使用迴歸型RF  
        ...
        for Sq=1:Split_quantity
            ...
            trainData=data(include_indices,nov);                %將數據集中未被選到做為驗證數據的數據存取出來
            trainLabels=labels(include_indices,1);              %將標籤集中未被選到做為驗證標籤的標籤存取出來
            validData=data(find(indices(:,:)==Sq),nov);         %將數據集中被選到做為驗證數據的數據存取出來
            validLabels=labels(find(indices(:,:)==Sq),1);       %將標籤集中被選到做為驗證標籤的標籤存取出來
...
elseif k_fold_switch==0 %不啟動k-fold功能
    ...
    if out_regression==1            %字串相符時確認使用迴歸型RF  
        trainData=data(:,nov);      %訓練集 (使用GA選擇的特徵); 維度=sample數*所選特徵數(numFeats)
        trainLabels=labels(:,1);    %訓練集其對應標籤 (使用GA選擇的特徵); 維度=sample數*1
