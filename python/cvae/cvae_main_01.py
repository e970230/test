...
class CVAE(nn.Module):
    def __init__(self, input_dim=784, num_classes=10, latent_dim=2, hidden_dim=400,hidden_dim_2=200):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_dim + num_classes, hidden_dim_2),
            nn.ReLU(),
...
for epoch in range(epochs):
    ...
    for x_batch, y_batch in train_loader:
        x_batch = x_batch.view(-1, 28 * 28).to(device)  # flatten
...
with torch.no_grad():
    ...
    y_label = torch.full((16,), 5, dtype=torch.long)  # 指定數字 5
...
for ax, img in zip(axes.flatten(), generated):
    ...
    ax.axis('off')
plt.suptitle("Generated Digits for Label 5")
plt.show()