...
# 此為透過使用pygad的基因演算法對sklearn的RandomForestRegressor進行超參數優化來使預測結果更精準
...
def fitness_func_method(ga_instance, solution, solution_idx):
    ...
    model = RandomForestRegressor(n_estimators=solution[0],
...
num_generations = 300                #疊代次數
num_parents_mating = 20              #交配染色體的數量
sol_per_pop = 30                     #染色體的數量
num_params = 30                      #想抓取的特徵數量(若不想抓取則輸入0，意即不抓取特徵使用當前所有特徵)
...
# 隨機森林的超參數範圍
gene_space = [
    {'low': 100, 'high': 500},   # n_estimators 樹的數量
...
for train_index_test_ver, test_index_test_ver in skf.split(test_data, label):
    ...
    test_model = RandomForestRegressor(n_estimators=solution[0],        # 樹的數量
                                       max_depth=solution[1],           # 樹的最大深度
                                       min_samples_split=solution[2],   # 葉節點最小樣本數
                                       random_state=42,
                                       n_jobs=-1)
