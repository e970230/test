...
num_params = 30                         #選擇的特徵數量，若不使用基因演算法選取特徵則此處請輸入0
num_genes = 3 + num_params              #欲求解數量；欲求解數量 = RF欲求超參數數量 + 欲求特徵數量 (ex:若我想使用RF並從原始數據中抓取30個特徵，則變數數量則為 3 + 30 = 33)
                                        #ET的3個欲求變數分別為 : n_estimators = 樹的數量；max_features = 分裂時考慮的最大特徵數 ； min_samples_split = 葉節點最小樣本數。
...
# 設置選擇特徵的基因範圍，若想選擇50個特徵則會創建一個50個{'low': 1, 'high': data的特徵上限}+前面設置的染色體範圍的配置
