...
def generate_specific_label_data_all_ver(model, x_target):
    model.eval()
    with torch.no_grad():
        all_mu, all_logvar = [], []
        for data in x_target:
            mu, logvar = model.encode(data.unsqueeze(0).to(device))
        ...
        global_mu = np.mean(all_mu, axis=0)
        global_std = np.sqrt(np.mean(np.exp(all_logvar), axis=0))
        
        latent_samples = np.random.normal(global_mu, global_std, size=(len(x_target), latent_dim)).astype(np.float32)
        latent_samples = torch.tensor(latent_samples).to(device)
        generated = model.decode(latent_samples).cpu().numpy()
...
generated = generate_specific_label_data_all_ver(model,x_train_scaled_test)
