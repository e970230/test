...
mat_data = scipy.io.loadmat('input_data_models_1_dlp_0_F_10000.mat')
x_train = mat_data['Final_input_data']  # 載入變數
...
latent_dim = 16     #encode 輸出層維度 decode 輸入層維度
...
model.eval()
with torch.no_grad():
    # 產生 m 筆隨機 latent vector (使用標準正態分布)
    m = 496
    latent_vectors = torch.randn(m, latent_dim).to(device)
    generated = model.decode(latent_vectors)
