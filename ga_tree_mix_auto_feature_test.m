...
lb_input=[10 5 5];         %超參數猜值下限
ub_input=[200 50 10];     %超參數猜值上限
...
%%
if Y_sample==0
    % 定義目標函數，x裡包含所有的猜值(森林裡決策樹的數量、每顆樹最大的分割次數、葉節點最小樣本數
...
if Y_sample==0
    ...
    for pl=1:size(Population_answer,3)  %將最佳解(適性值、最佳的超參數配合)從相對應的疊代裡撈出來
        ...
        final_answer(pl,:)=Population_answer(answer_index(pl),[1:Y_answer end],pl);
    end
